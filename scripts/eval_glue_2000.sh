...
eval_sst2() {
  hp_policy="$PWD/schedules/sst2_search_all_augm_2000.txt"
  local_dir="$PWD/results/"
  data_path="$PWD/datasets/"
  name="eval_sst2_schedule_2000"
  size=2000
  dataset="sst2"

  python3.6 pba/train.py \
    --local_dir "$local_dir" --data_path "$data_path" \
    --model_name bert --dataset "$dataset" --expsize "2000" \
...
eval_mnlimm() {
  hp_policy="$PWD/schedules/mnli_mm_search_all_augm_2000.txt"
  local_dir="$PWD/results/"
  data_path="$PWD/datasets/"
  name="eval_mnli_mm_schedule_2000"
  size=2000
  dataset="mnli_mm"
