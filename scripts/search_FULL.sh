...
sst2_search() {
    ...
    --model_name bert --dataset sst2 --expsize "FULL" \
    --train_size 1000 --val_size 8000 \
    --checkpoint_freq 0 --development --max_seq_length 128 \
    --name "sst2_search_all_augm_FULL" --gpu 1 --cpu 4 \
...
mnlimm_search() {
    ...
    python3.6 pba/search.py \
    --local_dir "$local_dir" --data_path "$data_path" \
    --model_name bert --dataset mnli_mm --expsize "FULL" \
    --train_size 1000 --val_size 8000 \
    --checkpoint_freq 0 --development --max_seq_length 128 \
    --name "mnli_mm_search_all_augm_FULL" --gpu 1 --cpu 4 \
    --num_samples 16 --perturbation_interval 1 --epochs 48 \
    --explore default_explore_fct --aug_policy default_aug_policy \
