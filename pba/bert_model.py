...
def build_bert_model(input_ids, input_mask, token_type_ids, num_classes, is_training, bert_config, noise_vector):
    use_one_hot_embeddings = True
    scope = 'bert'
    ...
    # --- added by the LAS authors
    embedding_output_aug = embedding_output + noise_vector
    ...
    if num_classes < 100:
        num_output_dim = num_classes
    else:
        num_output_dim = 1

    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        output_weights = tf.get_variable(
            "output_weights", [num_output_dim, hidden_size],
            initializer=tf.truncated_normal_initializer(stddev=0.02))

    output_bias = tf.get_variable(
        "output_bias", [num_output_dim], initializer=tf.zeros_initializer())
